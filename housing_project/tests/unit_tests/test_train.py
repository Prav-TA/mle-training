...
from src.housing_price_prediction.train import (
    train_forest_with_grid_search,
    train_linear,
    train_tree,
)
...
def test_train_linear(sample_X, sample_y):
    """
    Test the training functions for different regression models.

    This function tests the `train_linear`, `train_tree`, and
    `train_forest_with_grid_search` functions by ensuring that the
    models returned by these functions have a `predict` method.

    Args:
        sample_X (pd.DataFrame): The input features for training the models.
        sample_y (pd.Series): The target variable for training the models.
    """
    lin_model = train_linear(sample_X, sample_y)
    tree_model = train_tree(sample_X, sample_y)
    forest_model = train_forest_with_grid_search(sample_X, sample_y)

    assert hasattr(lin_model, "predict")
    assert hasattr(tree_model, "predict")
    assert hasattr(forest_model, "predict")
